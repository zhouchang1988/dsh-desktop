...
const localePatch = '@deepseek-ai+dsh-client-locale+0.1.0-rc.6.patch'
...
describe('zh dictionary mixed-language fixes', () => {
  it('translates the Cordis panel zh copy', async () => {
    const patch = await readFile(
      patchPath('@deepseek-ai+dsh-client-ui-cordis+0.1.0-rc.6.patch'),
      'utf8'
    )
  ...
  it('translates the reasoning-effort default label', async () => {
    const patch = await readFile(
      patchPath('@deepseek-ai+dsh-client-ui-model-selection+0.1.0-rc.6.patch'),
      'utf8'
    )
  ...
  it('translates the provider ID label in the models settings', async () => {
    const patch = await readFile(
      patchPath('@deepseek-ai+dsh-client-ui-settings-models+0.1.0-rc.6.patch'),
      'utf8'
    )
  ...
  it('translates the Awesome preset entry zh copy', async () => {
    const patch = await readFile(
      patchPath('@deepseek-ai+dsh-client-ui-agent-preset+0.1.0-rc.6.patch'),
      'utf8'
    )
  ...
  it('localizes the trajectory panel through tPanel', async () => {
    const patch = await readFile(
      patchPath('@deepseek-ai+dsh-client-ui-trajectory+0.1.0-rc.6.patch'),
      'utf8'
    )
